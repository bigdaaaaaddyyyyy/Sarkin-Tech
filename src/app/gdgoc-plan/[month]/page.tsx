  ...
  return chunks.map((chunk, index) => {
    ...
    if (chunk.startsWith("`") && chunk.endsWith("`")) {
      return (
        <code
          key={`${chunk}-${index}`}
          className="rounded-md bg-slate-100 px-1.5 py-0.5 text-sm text-slate-800"
        >
          {chunk.slice(1, -1)}
        </code>
      );
    ...
    nodes.push(
      <ul key={`list-${nodes.length}`} className="my-5 space-y-3">
        {listItems.map((item) => (
          <li
            key={item}
            className="flex gap-3 rounded-2xl bg-white p-4 text-slate-600 shadow-sm ring-1 ring-slate-100"
          >
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.startsWith("# ")) {
      nodes.push(
        <h1
          key={index}
          className="font-clash text-4xl font-bold tracking-tight text-slate-950 sm:text-6xl"
        >
    ...
    if (trimmed.startsWith("## ")) {
      nodes.push(
        <h2
          key={index}
          className="mt-10 font-clash text-2xl font-bold text-slate-950 sm:text-3xl"
        >
      ...
      <section className="relative mx-auto max-w-5xl">
        ...
        <div className="mt-8 rounded-[2.5rem] border border-white/80 bg-white/85 p-6 shadow-[0_30px_100px_rgba(15,23,42,0.09)] backdrop-blur-xl sm:p-10">
          <div className="mb-8 flex flex-col justify-between gap-4 border-b border-slate-200 pb-8 sm:flex-row sm:items-end">
            <div>
              <div
                className="mb-4 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-black text-white"
            ...
            <div className="shrink-0 rounded-3xl bg-slate-950 p-5 text-white sm:w-56">
              ...
              <p className="mt-2 break-words font-mono text-xs leading-6 text-white/80">
                content/gdgoc-plan/months/{meta.slug}.md
              </p>
            </div>
          </div>
        </div>
